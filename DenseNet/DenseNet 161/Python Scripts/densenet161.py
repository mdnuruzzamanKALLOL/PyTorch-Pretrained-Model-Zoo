...
class DenseNet(nn.Module):
    def __init__(self,growth_rate=48,block_config=(6,12,36,24),num_init_features=96,bn_size=4,drop_rate=0.0,num_classes=1000):
        super().__init__()
...
def densenet161(num_classes=1000): return DenseNet(growth_rate=48,block_config=(6,12,36,24),num_init_features=96,num_classes=num_classes)

def count_parameters(model, name="DenseNet-161"):
    ...
    for name_, p in model.named_parameters(): print(f"{name_:<55} {str(list(p.shape)):<25} {p.numel():>10,}")
    print("-"*92); print(f"{'TOTAL':<81} {total:>10,}")

if __name__=="__main__":
    model=densenet161(); print(model)
