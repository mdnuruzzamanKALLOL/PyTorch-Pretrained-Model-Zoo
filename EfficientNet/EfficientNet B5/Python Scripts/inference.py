...
WEIGHTS     = sys.argv[3] if len(sys.argv) > 3 else 'efficientnet_b5_best.pth'
CLASS_FILE  = sys.argv[2] if len(sys.argv) > 2 else None
IMAGE_PATH  = sys.argv[1]
TOP_K       = 5

transform = transforms.Compose([
    transforms.Resize(488),
...
model = efficientnet_b5(num_classes=NUM_CLASSES).to(DEVICE)
...
print(f'\nEfficientNet-B5 — Inference Results')
print('=' * 45)
BAR_LEN = 30
