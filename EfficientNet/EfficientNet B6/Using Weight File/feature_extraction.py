...
in_features = model.classifier[1].in_features   # 2304
...
# ── Optimizer (classifier head only) ─────────────────────────────
...
for epoch in range(1, EPOCHS + 1):
    ...
    if vl_acc > best_val_acc:
        best_val_acc = vl_acc
        torch.save(model.state_dict(), 'efficientnet_b6_fe.pth')

print(f'\nBest Val Acc : {best_val_acc:.2f}%')
print('Saved        : efficientnet_b6_fe.pth')
