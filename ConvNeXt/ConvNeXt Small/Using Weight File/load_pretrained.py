...
def get_labels():
    return models.ConvNeXt_Small_Weights.IMAGENET1K_V1.meta["categories"]
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python load_pretrained.py <image_path>"); sys.exit(1)
    print(f"Device : {DEVICE}  |  Model : ConvNeXt-Small (ImageNet weights)")
    model  = load_model()
    labels = get_labels()
    predict_top5(model, preprocess(sys.argv[1]), labels)
